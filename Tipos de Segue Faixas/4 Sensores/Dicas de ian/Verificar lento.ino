...
void loop()
{
    ...
    else if (leitura == 0b0011) // Condição 4
    {
        ...
        {
            enc_ant = enc.read();
            while (enc.read() - enc_ant <= enc_pas_p)
            {
                mot1_hor(vel_dir);
                mot2_hor(vel_dir);
                Serial.print("Indo devagar: ");
                Serial.println(enc.read());
            }
            mot1_par();
            mot2_par();
            delay(10);
            if(digitalRead(s_oeste) == 0 || digitalRead(s_noroeste) == 0){
                esquerda = true;
            }
        }
        identif_simpl();
    ...
    else if (leitura == 0b1100) // Condição 7
    {
        ...
        */
        ...
        for (int i; i <= 300; i++)
        {
            ...
            mot2_par();
            delay(10);
            if(digitalRead(s_leste) == 0 || digitalRead(s_nordeste) == 0){
                direita = true;
            }
        }
        identif_simpl();
